...
export default function SignedDocumentDisplay({}: {}) {
  ...
  if (!data) return <></>;
  return (
    <section className="py6 container mx-auto flex flex-col-reverse items-start gap-6 p-6 lg:flex-row">
      <Card className="mx-auto w-full flex-1 rounded-md bg-white p-6">
        <div>
      ...
      {data.request_status == "inprogress" && !isEmbedded && (
        <div className="w-full lg:sticky lg:top-[5em] lg:w-[20em]">
          <SendReminderCard />
        </div>
      )}
    </section>
